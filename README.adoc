=== Current Issues to verify on redeployment
The hosts are not populated in /etc/hosts (exercise 1.1-step 2)
The RHEL targets are getting the names "node1", "node2", "node3" instead of rhel7/8/9

Add the ALL_rhel group to the inventory in AAP

RHEL 8 needs cockpit-leapp installed
RHEL 8 host is on 8.7 instead of 8.10

RHEL 9 host is missing the leapp and cockpit-leapp packages
Update the images in 1.3 when the hostnames get corrected
