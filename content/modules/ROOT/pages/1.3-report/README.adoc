...
image::remote_host_menu_with_pets.png[Remote host menu listing all pet app servers,link=self,window=blank]
...
+
image::rhel8_os.png[upward-moray running Red Hat Enterprise Linux 8.10 (Ootpa),link=self,window=blank]
+
Here is an example of one running RHEL7:
+
image::rhel7_os.png[Operating System Red Hat Enterprise Linux Server 7.9 (Maipo),link=self,window=blank]
...
+
image::rhel8_report.png[Example pre-upgrade report of RHEL8 host,link=self,window=blank]
+
____
*Note*
...
+
image::grub_core_finding.png[Details view of grub core finding,link=self,window=blank]
+
...
+
image::remote_root_logins.png[Details view of remote root logins finding,link=self,window=blank]
+
...
host. For example, the report might look like this:
+
image::rhel7_report.png[Example pre-upgrade report of RHEL7 host,link=self,window=blank]
+
____
*Note*
...
+
image::rhel7_python.png[Details view of Difference in Python versions and support in RHEL8 finding,link=self,window=blank]
+
...
+
image::rhel7_cifs.png[Details view of cifs inhibitor,link=self,window=blank]
+
...
image::rhel9_report.png[Example pre-upgrade report of RHEL9 host,link=self,window=blank]


. In the report for our RHEL9 app server above, we see there are only two high risk findings.
One is the same GRUB2 core update findings we saw in the RHEL8 and RHEL7 pre-upgrade reports,
and there's not much new here.
. The other high risk finding is also not an inhibitor, but we'll take a look at it closely.
+

image::rhel9_unknown_repo.png[Details view of unknown repository finding,link=self,window=blank]

+
This finding is indicating that there's a repository configured on the system that is not 
recognized as a standard Red Hat repository.  This could be a custom or third-party repository, 
or a renamed Red Hat repository.  In any case, this could lead to issues during the upgrade if 
packages are pulled from an unsupported source.  We should investigate this repository further 
to determine if it's safe to proceed with the upgrade or if we need to disable it first.  
For the purposes of this lab, it's a Red Hat repository that has been renamed, so we can proceed.
...
+
image::rhel9_selinux.png[Details view of SELinux permissive,link=self,window=blank]
+
This finding is a low risk issue and it's not an inhibitor.  This finding is low, because 
the impact to the upgrade process is minimal. However, in many environments, this is a big deal, 
so we must take note of it as part of our post-upgrade checklist and re-enable SELinux after 
the upgrade!
