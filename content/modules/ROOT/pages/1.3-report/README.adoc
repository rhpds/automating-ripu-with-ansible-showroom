...
* If we were using the Leapp framework to manually upgrade just a
...
1.1, Step 2], we learned how to open an ssh session to your pet
...
reports. Do this by clicking the "lab-user​@​ansible-1" box
in the top left corner of the RHEL Web Console to reveal the remote host
menu. For example:
+

image::remote_host_menu_with_pets.svg[Remote host menu listing all pet app servers,link=self,window=blank]
...
+
image::rhel8_os.svg[upward-moray running Red Hat Enterprise Linux 8.10 (Ootpa),link=self,window=blank]
+
Here is an example of one running RHEL7:
+
image::rhel7_os.svg[Operating System Red Hat Enterprise Linux Server 7.9 (Maipo),link=self,window=blank]

. When you navigate to different hosts in the RHEL Web Console, look out
for the "limited access mode" warning:

+
image::limited_access.svg[Web console is running in limited access mode,link=self,window=blank]
...
+
image::administrative_access.svg[You now have administrative access,link=self,window=blank]
...
pre-upgrade reports. Let's start by looking at the RHEL8 host
and then we'll look at the other hosts in the next step.
...
image::ripu-workflow-hl-review.svg[Automation approach workflow diagram with review report step highlighted,link=self,window=blank]
...
. Having verified you are looking at your RHEL8 pet app server,
...
+
image::rhel8_report.svg[Example pre-upgrade report of RHEL8 host,link=self,window=blank]
+
____
*Note*
...
https://github.com/rhpds/automating-ripu-with-ansible-showroom/issues/new[here^].
____
...
from highest risk to lowest. In the report above, we see there are two
high risk findings. Let's review each of these.

. You can see additional details for any finding by clicking on it in
the list. For example, click on the first finding and you will see the
details like this:

+
image::jitterentropy-devel.svg[Packages available in excluded repositories will not be installed finding,link=self,window=blank]

+
The first finding is blah blah. leapp is actually telling you/customer
that it's ignoring that package. what is the problem then?

+
[11:51 AM]you see the msg because the system has installed rng-tools rpm
which has been split to multiple rpms on RHEL 9
(https://pes.osci.redhat.com/?fromRelease=all&toRelease=all&search=jitterentropy-devel&detail=8811)
and one of them is in CRB repository. obviously, customer will most
likely not be interested in the devel package, but if so, they know it's
there and what to do in case it should be installed (and where it's
actually located)

+
[11:52 AM](/me is going offline for couple of hours)

+
[11:53 AM]in case you just do not want to see this rpm reported for any
reason, I think it could be resolved if you specify it in the
/etc/leapp/files/transaction/to_remove file. I am not 100% sure right
now, so I suggest to test it manually, but it could work.

. The next finding we see listed has the title "GRUB2 core will be
automatically updated during the upgrade." 

+
image::grub_core_finding.svg[Details view of grub core finding,link=self,window=blank]

+
This finding is reported on every upgrade, as it is potentially
impactful. The summary provides the details of the issue and the
recommended action to take. We'll ignore this one for now, but make a
note of it for later.

. The other findings on our RHEL8 report are not marked as high, but are
still worth understanding. We will go into some of these more later in
this exercise.

* As we saw here, there's a lot of information in the report, and it can
be overwhelming.  The good news is that we can filter the findings to
make it easier to focus on the most important ones.
...
+
Let's now move on to the pre-upgrade report for your RHEL7
hosts.

[#step-4-review-leapp-pre-upgrade-report-of-rhel7-host]
=== Step 4 - Review Leapp Pre-upgrade Report of RHEL7 Host

In the previous step, we reviewed the pre-upgrade report for your
RHEL8 host. Now let's take a look at the report from your RHEL7
host.

. Navigate to the RHEL Web Console remote host menu and click on the
hostname of your RHEL7 pet app server. Verify the host you have
...
+
image::rhel7_report.svg[Example pre-upgrade report of RHEL7 host,link=self,window=blank]
...
____
. In the report for our RHEL7 pet app server above, we see there are many more findings we need to address. Let’s start
...
+
image::rhel7_python.svg[Details view of Difference in Python versions and support in RHEL8 finding,link=self,window=blank]
+
...
. The other new high risk finding, and also an inhibitor, is the "Missing required answers in the answer
file" finding. Here are the details for this one:
+
image::missing_answers_inhibitor.svg[Details view of missing required answers in the answer file,link=self,window=blank]
+
...
+
image::rhel7_cifs.svg[Details view of cifs inhibitor,link=self,window=blank]
+
...
* This may be concerning to see so many high risk findings on the RHEL7
pre-upgrade report.  However, as we look into remediating these findings in
the next exercise, we will see how easy it is to address them with automation.
...
Following the same steps as in the previous two exercises, let's now take a look
at the report from our RHEL9 host.

image::rhel9_report.svg[Example pre-upgrade report of RHEL9 host,link=self,window=blank]


. In the report for our RHEL9 app server above, we see there is only 1 high risk finding.
It's the same GRUB2 core update findings we saw in the RHEL8 and RHEL7 pre-upgrade reports,
and there's not much new here.

. However, lets take a look at the rest of the findings. There's a
finding that is marked as a low risk, "SElinux will be set to
permissive mode."

+
image::rhel9_selinux.svg[Details view of SELinux permissive,link=self,window=blank]

+
This finding is a low risk issue and it's not an inhibitor.  This
finding is low because the impact to the upgrade process is minimal.
However, in many environments, this is a big deal, so we must take note
of it as part of our post-upgrade checklist and re-enable SELinux after
the upgrade!
