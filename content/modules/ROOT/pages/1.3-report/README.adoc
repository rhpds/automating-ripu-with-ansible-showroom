...
image::remote_host_menu_with_pets.png[Remote host menu listing all pet app servers]
...
+
image::rhel8_os.png[upward-moray running Red Hat Enterprise Linux 8.10 (Ootpa)]
+
Here is an example of one running RHEL7:
+
image::rhel7_os.png[Operating System Red Hat Enterprise Linux Server 7.9 (Maipo)]
...
+
This finding is reported on every upgrade, as it is potentially impactful.  The summary provides the details of the issue and the recommended action to take.  As we see, there's no action required, so we can move on.
+
image::remote_root_logins.png[Details view of remote root logins finding]
+
...
. Now, looking at the inhibitors, first is the "Missing required answers in the answer
...
+
image::rhel7_cifs.png[Details view of cifs inhibitor]
+
...
image::rhel9_unknown_repo.png[Details view of unknown repository finding]
