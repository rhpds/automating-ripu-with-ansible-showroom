...
The lab environment is accessed via a bastion host that is running the
services to support the workshop experience. The lab consists of an
Ansible Automation Platform (AAP) controller host and three RHEL pet
application hosts, one each for RHEL7, RHEL8, and RHEL9. These pet app
hosts are where we will be performing the RHEL in-place upgrades. The
AAP controller host executes the Ansible playbook and workflow jobs that
automate the RHEL in-place upgrade process.

image::ripu_lab_diagram.svg[RHEL In-place Upgrade Automation Workshop lab diagram,link=self,window=blank]
...
* xref:1.5-custom-modules/README.adoc[Exercise 1.5 - (Optional) Custom Pre-upgrade Checks]
* xref:1.6-my-pet-app/README.adoc[Exercise 1.6 - (Optional) Deploy a Pet App]
