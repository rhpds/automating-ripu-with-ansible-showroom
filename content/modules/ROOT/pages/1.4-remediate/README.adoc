...
We are now going to look at the remediations step of our automation
approach workflow:
...
were generated for our RHEL pet application servers. Our RHEL7 server
had a number of inhibitors that will require attention before the RHEL
upgrade can proceed.

. Let's start by dissecting one of our inhibitor findings:

+
image::missing_answers_dissected.svg[Details view of missing required answers in the answer file,link=self,window=blank]

+
image:circle_1.svg[1.,link=self,window=blank] Each finding has a unique
title.

+
image:circle_2.svg[2.,link=self,window=blank] A risk factor is assigned
to each finding, but as we discussed in the previous exercise, this may
be more nuanced than can be indicated by a simple High, Medium, Low or
Info rating.

+
image:circle_3.svg[3.,link=self,window=blank] The summary provides a
detailed explanation of the risk and solution recommendation.

+
image:circle_4.svg[4.,link=self,window=blank] Under remediation, we are
given a fairly prescriptive recommendation.

+
image:circle_5.svg[5.,link=self,window=blank] Sometimes, the remediation
also includes an exact command like this one.
...
"Run Remediation" button shown above the command. Using this option, the
RHEL Web Console executes the command for us. While doing this is less
prone to human error, it still doesn't scale well as it's only going to
run on this single host.
...
https://leapp.readthedocs.io/en/stable/tutorials/dialogs.html#asking-user-questions[Asking user
...
For example:
+
image::analysis_leapp_answerfile.svg[Setting the `leapp_answerfile` input variable,link=self,window=blank]
+
...
+
image::analysis_survey_rhel7_only.svg[Choose "rhel7" at the survey prompt,link=self,window=blank]
+
Click the "Next" button to proceed to the preview prompt. If you are
satisfied with the job preview, use the "Finish" button to start the
job.
...
For example:
+
image::rhel7_answer_fixed.svg[Pre-upgrade report of RHEL7 host without answer file inhibitor,link=self,window=blank]
+
...
. Here is our finding:
+
image::root_account_inhibitor.svg[Details view of missing required answers in the answer file,link=self,window=blank]
+
Like the previous inhibitor finding, this one also provides a detailed
summary and a fairly prescriptive recommended remediation. However, it
...
. Of course, we're not going to just log in to a root shell and `vi` the
    ...
    - name: Include the leapp remediate role
      ...
      vars:
        remediation_todo:
          ...
          - leapp_cifs_detected
----
This remediation approach uses the `remediate` role from the `infra.leapp`. Let's look at the documentation for this role at https://github.com/redhat-cop/infra.leapp/tree/main/roles/remediate[infra.leapp remediate role^]. We can see that this role supports a `remediation_todo` variable that accepts a list of remediation tasks to perform.  Each of the items in the list corresponds to a specific remediation task defined by the role.  In our example above, we are including remediation tasks for all the inhibitors we have seen so far in our pre-upgrade reports.

. For our purposes, the "OS / Remediation" job template is already set up to execute this
playbook, so let’s use it to remediate our RHEL7 hosts.
. Return to your AAP Web UI browser tab. Navigate to Automation Execution >
Templates on the AAP Web UI and open the "OS / Remediation" job
template. Click the "Launch" button to get started.
. This will bring you to the job template survey prompt. Click "Next" at the variables 
survey question then choose the "rhel7" option at the "Select inventory group" prompt 
because our remediation playbook is specific to the pre-upgrade findings of our RHEL7 
hosts. Then click the "Next" button. If you are satisfied with 
the job preview, use the "Finish" button to submit the job. This
playbook includes only a small number of tasks and should run pretty
quickly.
. When the "OS / Remediation" job is finished, launch the "AUTO / 01
...
+
image::rhel7_no_inhibitors.svg[Pre-upgrade report of RHEL7 host with no more inhibitors,link=self,window=blank]
+
