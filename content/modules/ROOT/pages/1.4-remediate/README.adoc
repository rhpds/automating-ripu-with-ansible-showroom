...
were generated for our RHEL pet application servers. Each server had a couple 
of inhibitors that will require attention before the RHEL upgrade can proceed.
...
+
image:circle_1.svg[1.,link=self,window=blank] Each finding has a unique title.
+
image:circle_2.svg[2.,link=self,window=blank] A risk factor is assigned to each finding,
...
+
image:circle_3.svg[3.,link=self,window=blank] The summary provides a detailed
explanation of the risk and solution recommendation.
+
image:circle_4.svg[4.,link=self,window=blank] Under remediation, we are given a fairly
prescriptive recommendation.
+
image:circle_5.svg[5.,link=self,window=blank] Sometimes, the remediation also includes
...
For example:
+
image::analysis_leapp_answerfile.png[Setting the `leapp_answerfile` input variable,link=self,window=blank]
+
...
+
image::analysis_survey_rhel7_only.png[Choose "rhel7" at the survey prompt,link=self,window=blank]
+
...
For example:
+
image::rhel7_answer_fixed.png[Pre-upgrade report of RHEL7 host without answer file inhibitor,link=self,window=blank]
+
