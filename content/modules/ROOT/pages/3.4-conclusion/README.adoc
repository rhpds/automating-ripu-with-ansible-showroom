...
* You upgraded only a handful of RHEL instances while progressing
...
from there to RHEL9. You can try this with the pet app instance that was
originally running RHEL7. You will need to roll it back to RHEL7 first.
Then take a look at the "OS / Upgrade 7-8-9" workflow job template.

+
One consideration with going from RHEL7 to RHEL9 is the increased
...
sure is to try it! Remember, fail fast!
...
==== https://github.com/rhpds/leapp-project[rhpds/leapp-project^]
...
https://github.com/rhpds/automating-ripu-with-ansible-showroom/issues/new[here] tagging
@swapdisk and @heatmiser.
