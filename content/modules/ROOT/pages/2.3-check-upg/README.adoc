...
For example:
+
image::upgrade_jobs.png[AAP Web UI listing upgrade job entries]
...
+
image::upgrade_play_recap.png[AAP Web UI showing successful upgrade playbook run play recap]
+
...
+
image::upgrade_failure.png[Searching for "failed" in the upgrade playbook log output]
+
...
reporting RHEL9:
+
image::rhel9_upgraded.png[node2 running Red Hat Enterprise Linux 9.2 (Plow)]
+
Here is an example one that was previously RHEL7 now running RHEL8:
+
image::rhel8_upgraded.png[node1 running Red Hat Enterprise Linux Server 8.8 (Oopta)]
+
And even our RHEL 9 server, which did showed us an error in the upgrade playbook, is now running RHEL 10:
+
image::rhel10_upgraded.png[node3 running Red Hat Enterprise Linux 10.0 (Plow)]
+
...
+
image::rhel10_commands.png[Command line showing RHEL 10.0 and kernel version 6.12.0]
