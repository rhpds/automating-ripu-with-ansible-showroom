...
For example:
+
image::upgrade_jobs.svg[AAP Web UI listing upgrade job entries,link=self,window=blank]
...
+
image::upgrade_play_recap.svg[AAP Web UI showing successful upgrade playbook run play recap,link=self,window=blank]
+
...
+
image::rhel9_upgraded.svg[RHEL web console running Red Hat Enterprise Linux 9.7 (Plow),link=self,window=blank]

+
Here is an example of one that was previously RHEL7 now running RHEL8:

+
image::rhel8_upgraded.svg[RHEL web console running Red Hat Enterprise Linux Server 8.10 (Oopta),link=self,window=blank]

+
And here is our RHEL9 server now running RHEL10:

+
image::rhel10_upgraded.svg[RHEL web console running Red Hat Enterprise Linux 10.1 (Plow),link=self,window=blank]
...
showing the pet app server that was upgraded to RHEL10:

+
image::rhel10_commands.svg[Command line showing RHEL 10.1 and kernel version 6.12.0,link=self,window=blank]

+
We can also use the `last -w reboot` command to demonstrate that the
host had been previously booting the kernel version from the old RHEL
major version:

+
image::reboot_history.svg[last command output showing reboot history,link=self,window=blank]
