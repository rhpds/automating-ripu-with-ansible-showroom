...
* link:#workshop-exercise---run-os-upgrade-jobs[Workshop Exercise - Run OS Upgrade Jobs]
** link:#table-of-contents[Table of Contents]
** link:#objectives[Objectives]
** link:#guide[Guide]
*** link:#step-1---launch-the-upgrade-workflow-job-template[Step 1 - Launch the Upgrade Workflow Job Template]
*** link:#step-2---learn-more-about-leapp[Step 2 - Learn More About Leapp]
...
image::images/ripu-workflow-hl-upgrade.svg[Automation approach workflow diagram with upgrade step highlighted]
...
+
image::images/upgrade_template.svg[AAP Web UI showing the upgrade job template details view]
* Click the ``Launch'' button which will bring up a the prompts for submitting the job starting with the variables prompt:
+
image::images/upgrade_vars_prompt.svg[Upgrade job variables prompt on AAP Web UI]
* We don’t need to change the variables settings, so just click the ``Next'' button to move on.
+
image::images/upgrade_survey_prompt.svg[Upgrade job survey prompt on AAP Web UI]
...
+
image::images/inplace-upgrade-workflow-gbg.svg[Leapp upgrade flow diagram]
...
* Being aware of these phases helps if you need to troubleshoot an issue during the Leapp upgrade and is especially important if you are planning to develop any Leapp custom actors. You can learn more about the Leapp framework architecture and internal design by reading the upstream https://leapp.readthedocs.io/en/latest/index.html[Leapp Developer Documentation].
...
link:../1.6-my-pet-app/README.md[Previous Exercise] - link:../2.2-snapshots/README.md[Next Exercise]
