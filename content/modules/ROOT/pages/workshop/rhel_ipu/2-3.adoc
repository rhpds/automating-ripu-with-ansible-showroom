...
* link:#workshop-exercise---check-if-the-upgrades-worked[Workshop Exercise - Check if the Upgrades Worked]
** link:#table-of-contents[Table of Contents]
** link:#objectives[Objectives]
** link:#guide[Guide]
*** link:#step-1---review-the-upgrade-playbook-job-log[Step 1 - Review the Upgrade Playbook Job Log]
*** link:#step-2---verify-the-hosts-are-upgraded-to-next-rhel-version[Step 2 - Verify the Hosts are Upgraded to Next RHEL Version]
...
image::images/ripu-workflow-hl-validate.svg[Automation approach workflow diagram with app validation steps highlighted]
...
For example:
+
image::images/upgrade_jobs.svg[AAP Web UI listing upgrade job entries]
...
+
image::images/upgrade_play_recap.svg[AAP Web UI showing successful upgrade playbook run play recap]
+
...
* In link:../1.3-report/README.md#step-2---navigating-the-rhel-web-console[Exercise 1.3: Step 2], you used the RHEL Web Console to check the installed RHEL versions on your pet app servers. Let’s repeat those steps to see the RHEL versions reported after our upgrades.
+
...
+
image::images/rhel9_upgraded.svg[fluent-bee running Red Hat Enterprise Linux 9.2 (Plow)]
+
Here is an example one that was previously RHEL7 now running RHEL8:
+
image::images/rhel8_upgraded.svg[vocal-hyena running Red Hat Enterprise Linux Server 8.8 (Oopta)]
* You can also check the RHEL and kernel versions from the command line following the steps you used with link:../1.1-setup/README.md#step-2---open-a-terminal-session[Exercise 1.1: Step 2].
+
...
+
image::images/rhel9_commands.svg[command output showing RHEL9 is installed]
...
* link:../3.1-rm-rf/README.md[Exercise 3.1 - (Optional) Trash the Instance]
...
link:../2.1-upgrade/README.md[Previous Exercise] - link:../2.4-check-pet-app/README.md[Next Exercise]
