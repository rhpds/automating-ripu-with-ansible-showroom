...
* link:#workshop-exercise---deploy-a-pet-app[Workshop Exercise - Deploy a Pet App]
** link:#table-of-contents[Table of Contents]
** link:#optional-exercise[Optional Exercise]
** link:#objectives[Objectives]
** link:#guide[Guide]
*** link:#step-1---the-traditional-application-lifecycle[Step 1 - The Traditional Application Lifecycle]
*** link:#step-2---installing-our-beloved-pet-application[Step 2 - Installing Our Beloved Pet Application]
*** link:#step-3---test-the-pet-application[Step 3 - Test the Pet Application]
*** link:#step-4---configure-the-application-to-start-on-reboot[Step 4 - Configure the Application to Start on Reboot]
*** link:#step-5---run-another-pre-upgrade-report[Step 5 - Run Another Pre-upgrade Report]
...
* Our example application will be the https://github.com/spring-projects/spring-petclinic[Spring Pet Clinic Sample Application] written in Java. It is a Spring Boot application that gets built using Maven. It will connect to a MySQL database which gets populated at startup with sample data.
+
____
*Note*
...
If the app install job is successful, you can skip ahead to link:#step-3---test-the-pet-application[Step 3 - Test the Pet Application].
____
...
+
image::images/packages_not_signed_by_rh.svg[Packages not signed by Red Hat found on the system high risk finding]
* If we open the finding, we are presented with the following details:
+
image::images/packages_not_signed_details.svg[Packages not signed by Red Hat details view]
...
link:../1.5-custom-modules/README.md[Previous Exercise] - link:../2.1-upgrade/README.md[Next Exercise]
